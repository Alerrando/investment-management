...
export default function TableROE() {
  const [sortConfig, setSortConfig] = useState<SortConfigProps>({ key: "roe", direction: "asc" });
  ...
  const sortedStocks = useMemo(() => {
    ...
    return sortedData;
  }, [sortConfig, dataListStocksByRoe.content]);
  ...
  return (
    <div className="z-30 min-w-[300px] flex-1 transform rounded-2xl border border-gray-200 bg-gradient-to-br from-white to-gray-50 p-6 shadow-lg transition-all hover:shadow-xl dark:border-[#444444] dark:from-[#2C2C2C] dark:to-[#1E1E1E]">
      <header className="mb-6 flex items-center justify-between">
        <h2 className="text-xl font-bold text-gray-900 dark:text-white">Ações com maior ROE</h2>
      </header>
      <div className="h-[350px] overflow-y-auto">
        <TableReusable.Root>
          ...
          <tbody>
            {dataListStocksByRoe.content.length > 0 ? (
              <>
                {sortedStocks.slice(0, 8).map((stock, index) => (
                  <TableReusable.TbRow key={index}>
                    ...
                    <TableReusable.TbRowTd stock={stock.pActive} />
                  </TableReusable.TbRow>
                ))}
              </>
            ) : (
